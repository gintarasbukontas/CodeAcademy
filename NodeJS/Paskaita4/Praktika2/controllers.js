...
export function getOwnersByCar(req, res) {
  const params = req.params;
  const carBrand = params.car;

  const foundOwners = data.filter((x) => x.car === carBrand);
...
export function getFemales(req, res) {
  const females = data.filter((x) => x.gender === "Female");

  const femaleArray = [];

  females.forEach((female) => {
    femaleArray.push(female.first_name + " " + female.last_name);
  });
  res.json(femaleArray);
}
